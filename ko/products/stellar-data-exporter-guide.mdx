...
5. 다음 항목을 확인합니다.
   ...
   - Event Name
   - 전체 Matching Count
6. Preview가 **0건**이면 예제 값이 현재 환경에 없는 것일 수 있습니다. 먼저 `*:*`로 Preview한 뒤 실제 데이터에 존재하는 `event_name`, `srcip` 값을 확인하고 그 값으로 조건을 다시 만드세요.
7. 결과가 너무 많으면 조건을 하나 더 추가합니다.
8. 원하는 데이터가 맞으면 **CSV**를 선택합니다.
...
1. Data Source에서 다음을 같이 선택합니다.
   ...
   - **Traffic**
   - **Windows Events**
2. 처음에는 짧은 Incident **Start** / **End** Time으로 시작합니다.
3. Incident에 필요한 조건으로 Query 범위를 충분히 좁힙니다.
4. **Preview 100 records**를 실행하고 **Matched records**를 확인합니다.
5. 예상보다 건수가 크면 Export하기 전에 Time Range를 더 줄이거나 Query 조건을 좁힙니다.
6. **Advanced**로 전환한 뒤 **Query Inspector**에서 **Resolved indices**와 Effective Query를 확인합니다.
7. Output은 **JSON**을 선택합니다.
8. 결과가 충분히 작을 때만 **Download**를 사용합니다. 결과가 크면 S3-Compatible Storage 또는 SFTP를 사용하세요.
9. **Run export**를 실행합니다.
...
1. **Alerts**를 선택합니다.
2. **7d** 버튼을 선택하거나 정확한 Start/End Time을 지정합니다.
3. Query를 입력합니다.
...
1. **Advanced**로 전환합니다. **Query Inspector**는 Query 영역 안에 자동으로 표시되며 별도의 Open 버튼은 없습니다.
2. 일반 Query 설정에서 선택한 Data Source와 Start/End Time을 확인합니다.
3. **Query Inspector**에서 다음을 확인합니다.
...
4. Matched Count 또는 Estimated Output Size가 너무 크면 Time Range를 줄입니다.
5. Preview를 다시 실행합니다.
