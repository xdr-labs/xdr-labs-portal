...
<Note>
  User Scope API Key도 Raw Data 검색과 Export에 사용할 수 있습니다. Preview 전용이나 Metadata 전용 Mode로 취급하지 않습니다. User API Key라도 실제 권한에 따라 여러 Tenant가 보일 수 있으므로 Tenant 선택은 항상 필요합니다.
</Note>
...
- 운영 배포에서는 `STELLAR_EXPORTER_UI_USERNAME`, `STELLAR_EXPORTER_UI_PASSWORD`로 개발 기본 Credential을 변경하세요.
