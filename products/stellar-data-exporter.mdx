...
- required one-or-more Data Source selection, with Alerts selected by default and internal Stellar Cyber index mapping
- explicit start/end time selection
- Elasticsearch DSL for Root Scope and Stellar Cyber/Lucene Query for Root Scope or User Scope
...
<Note>
  User Scope API Key mode supports raw-data search and export through **Stellar Cyber Query (Lucene)**. Elasticsearch DSL is intentionally disabled for User Scope and is available only with **Root Scope**. A User API Key can still expose multiple tenants depending on its effective Stellar Cyber permissions, so tenant selection is always required.
</Note>
...
<CardGroup cols={3}>
  <Card title="Browser download" icon="download">
    Export CSV, JSON, or NDJSON directly to the browser. The complete file is prepared before download starts, and split browser exports can be delivered as a ZIP.
  </Card>
  ...
  <Card title="SFTP" icon="arrow-right-arrow-left">
    Send output over SFTP with password or SSH private-key authentication, with SHA256 host-key discovery and explicit key pinning.
  </Card>
</CardGroup>
...
- direct self-hosted runs bind HTTPS to `0.0.0.0:8787` by default, so users on allowed networks connect with `https://<exporter-host-ip>:8787`; restrict TCP/8787 with the host firewall as needed
